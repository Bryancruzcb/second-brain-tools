...
| 3 | Done 2026-09-19. Prometheus, Grafana, dashboards as JSON, nine alert rules with promtool tests, the canary and its baseline card, and alerts as GitHub issues. Verified on the node the same day (`deploy/monitoring/README.md`, What the first run showed); the round trip to a real issue waits for the token |
