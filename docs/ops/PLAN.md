...
| 3 | Done 2026-09-19. Prometheus, Grafana, dashboards as JSON, nine alert rules with promtool tests, the canary and its baseline card, and alerts as GitHub issues. Verified on the node the same day, including an alert opening a GitHub issue and closing it again when it resolved (`deploy/monitoring/README.md`, What the first run showed) |
| 4 | Built 2026-09-19, not yet run on a node. `deploy.yml`: staging, the gate Job, promote, the smoke Job, rollback on any failure, and one DynamoDB item per deploy. Every cluster step is a script sent over SSM (`deploy/pipeline/`), and `send_command.py` checks what came back before it prints it, so output that names a `.md` file or carries a string where a number belongs is described rather than shown. `nightly-cost.yml` compares the running node with `OPS_STATE` every night and opens an issue when they disagree |
| 5 | Written 2026-09-19, not yet run. Four game-day scripts in `deploy/gameday/`, each measuring time to alert and time to recovery against `results.tsv`, which ships with its header and no rows; four runbooks in `deploy/runbooks/`; `docs/ops/postmortem-stale-index.md` for the stale-index incident scenario 3 replays; and `docs/ops/cost.md`, whose measured cells stay empty until a month closes. Writing them found two real gaps, both closed: `reindex.sh` (nothing could reindex a healthy namespace) and `status.sh` (nothing could look at the cluster read-only) |
| 6, three days | Docs done 2026-09-19: `deploy/README.md` is the front door for the whole system, `docs/ops/architecture.md` matches what was built (GitHub issues rather than SNS, the deploy and failure flows, the canary and smoke Jobs), and the root README has a section on the cloud deployment. Left: run the game day, fill `cost.md` and `results.tsv` from real runs, and destroy the environment and rebuild it with `python deploy/ops.py up` to prove it works. All three need the node |
