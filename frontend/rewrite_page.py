...
export default function Home() {
  ...
  return (
    <div className="dashboard-layout">
      ...
      {/* Sidebar Toggles */}
      <button className="sidebar-toggle left-btn" onClick={() => setIsLeftOpen(!isLeftOpen)} title="Toggle Dashboard">
        {isLeftOpen ? <PanelLeftClose size={20} /> : <PanelLeftOpen size={20} />}
      </button>

      <button className="sidebar-toggle right-btn" onClick={() => setIsRightOpen(!isRightOpen)} title="Toggle AI Copilot">
        {isRightOpen ? <PanelRightClose size={20} /> : <PanelRightOpen size={20} />}
      </button>

      {/* LEFT SIDEBAR: Dashboard Control Center */}
      <div className={`sidebar left ${isLeftOpen ? 'open' : 'closed'}`}>
        <div style={{ padding: '24px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '24px', height: '100%' }}>
          ...
          <div className="hero-brand" style={{ marginBottom: 0 }}>
            ...
            <div>
              <h1 className="hero-title" style={{ fontSize: '20px' }}>Second Brain</h1>
              <p className="hero-subtitle" style={{ fontSize: '12px' }}>Knowledge Engine</p>
            </div>
          </div>

          <button className="scan-btn" onClick={triggerScan} disabled={isScanning}>
          ...
          {healthData && brokenCount > 0 && (
            <div className="glass-card detail-card" style={{ marginTop: 0 }}>
              <div className="detail-card-header" style={{ marginBottom: 12 }}>
              ...
              <ul className="detail-list" style={{ padding: 0, margin: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 8 }}>
                {healthData.broken_links.slice(0, 10).map((bl, i) => (
                  <li key={i} style={{ display: 'flex', gap: 8, fontSize: 12, color: '#a0aab2' }}>
                    ...
                    <span style={{ color: 'var(--color-accent-rose)' }}>{bl.target}</span>
                  </li>
                ))}
              </ul>
            </div>
      ...
      <div className={`sidebar right ${isRightOpen ? 'open' : 'closed'}`} style={{ width: '420px' }}>
        {selectedNode ? (
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            {/* Top Half: Note Content */}
            <div style={{ flex: 1, padding: '20px', overflowY: 'auto', borderBottom: '1px solid var(--color-glass-border)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%', padding: '16px 0' }}>
            <h3 style={{ color: '#fff', fontSize: 16, margin: '0 16px 16px', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8 }}>
              <Brain size={18} style={{ color: 'var(--color-accent-purple)' }} /> AI Copilot
            </h3>
