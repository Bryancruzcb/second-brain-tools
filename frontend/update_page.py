...
export default function Home() {
  ...
  const handleSaveNote = async () => {
    ...
    setIsSaving(true);
    try {
      await fetch(`${API_BASE}/api/note/${encodeURIComponent(selectedNode.label)}`, {
        method: 'POST',
  ...
  const handleCowrite = async () => {
    setIsCowriting(true);
    try {
      const res = await fetch(`${API_BASE}/api/cowrite`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ content: editContent })
      });
      const data = await res.json();
      setEditContent(prev => prev + (prev.endsWith(' ') ? '' : ' ') + data.completion);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  // Render Note Editor UI Block
  const renderNoteEditorUI = () => (
    <>
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <button 
          ...
          style={{
            ...
            background: 'rgba(255, 255, 255, 0.05)', color: '#a0aab2', border: '1px solid rgba(255, 255, 255, 0.08)', borderRadius: '8px', padding: '8px',
        ...
        <button 
          ...
          style={{
            flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
            background: isEditing ? 'linear-gradient(135deg, #10b981, #059669)' : 'linear-gradient(135deg, #6366f1, #8b5cf6)',
            color: '#fff', border: 'none', borderRadius: '8px', padding: '8px',
            fontWeight: 600, cursor: 'pointer', fontSize: 13
          }}
        >
          {isSaving ? <Loader2 size={14} className="spin" /> : isEditing ? <Save size={14} /> : <Edit3 size={14} />}
          {isEditing ? 'Save Note' : 'Edit Note'}
        </button>
        {isEditing && (
          <button 
            onClick={handleCowrite} disabled={isCowriting}
            style={{
              flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
          ...
          >
            {isCowriting ? <Loader2 size={14} className="spin" /> : <Wand2 size={14} />}
            AI Co-write
          </button>
        )}
      </div>

      <div style={{ color: '#e2e8f0', fontSize: 13, lineHeight: 1.6, flex: 1, display: 'flex', flexDirection: 'column' }}>
        {isLoadingNote ? (
          ...
          <textarea 
            ...
            style={{ 
              flex: 1, width: '100%', minHeight: '300px', background: 'rgba(0,0,0,0.3)', 
              color: '#fff', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, 
    ...
    return (
      <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', overflow: 'hidden', display: 'flex', background: '#020205', zIndex: 9999 }}>
        ...
        <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', zIndex: 1 }}>
          <GraphCanvas nodes={healthData?.nodes} edges={healthData?.edges} onNodeSelect={setSelectedNode} isExpanded={isFullscreen} />
        </div>
        ...
        <div style={{
          ...
          {selectedNode ? (
            <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
              ...
              <div style={{ flex: 1, padding: '20px', overflowY: 'auto', borderBottom: '1px solid rgba(255, 255, 255, 0.08)', display: 'flex', flexDirection: 'column' }}>
